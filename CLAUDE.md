...
Obtenir l'approbation du produit de **production** Orange Money (le sandbox "WebPay Dev" ne couvre que les tests — voir plus haut), pour pouvoir repasser `ORANGE_MONEY_COUNTRY` sur le vrai code pays et vérifier un paiement réel de bout en bout avec de l'argent réel, y compris la forme exacte du payload webhook envoyé à `notif_url` (jamais vu en conditions réelles, seul l'appel API a été vérifié en sandbox). PayDunya reste une option de repli si son KYC aboutit avant. Puis enregistrer les URLs de webhook (Cloudflare, Orange Money/PayDunya) une fois l'API déployée sur Render (déjà fait, voir `infra/DEPLOY.md`) — l'IP autorisée sur le token Cloudflare Stream devra alors couvrir l'IP de sortie de Render, pas seulement celle utilisée pour la vérification manuelle. En parallèle, la feuille de route produit continue (items P2 : téléchargement hors-ligne, suivre un créateur, tri/réponse sur les avis, canal support viewer, squelette i18n bambara), et la publication Play Store avance (voir `infra/PLAY_STORE_LISTING.md`).
