...
**Audit sécurité/qualité à fond, 4 volets en parallèle** (2026-09-04) : API, web, mobile, docs/infra. Corrigés — un vrai **double-crédit du ledger** possible si `ConfirmPayment` était appelé deux fois en concurrence (webhook Orange Money rejoué), une **race condition sur les demandes de retrait** (deux requêtes concurrentes pouvaient réserver plus que le solde réel), le **token de session mobile stocké en clair** (migré vers `flutter_secure_storage`), un **401 jamais géré côté web** (session qui restait "connectée" indéfiniment après expiration/suspension), un **open-redirect** sur `?next=` après connexion, plus une dizaine de petits bugs de robustesse (`setState` après démontage, lecteur vidéo bloqué en spinner infini, chargements qui échouaient silencieusement sans état "réessayer"). Détail complet dans l'historique git (commits "Audit …", 2026-09-04). Suite : 159/159 backend, `tsc`/`lint`/`build` web propres, `flutter analyze`/`test` mobile propres.

**Domaine dédié** (2026-09-06) : bascule de `onrender.com` vers `streammali.ml` (+ `www` + `api`), domaine vérifié et certificat émis. URLs par défaut remplacées dans l'app mobile, la fiche Play Store et `NEXT_PUBLIC_API_URL` ; variables à mettre à jour manuellement dans le dashboard Render (`APP_URL`, `CORS_ALLOWED_ORIGINS`, les 3 URLs Orange Money, `NEXT_PUBLIC_SITE_URL`) documentées dans `infra/DEPLOY.md`.

**Feuille de route P2 — deux items de plus faits** (2026-09-08) :
- **Téléchargement hors-ligne, mobile uniquement** (choix délibéré : un téléchargement web finirait dans le dossier standard du navigateur, trivialement copiable, à l'encontre de la protection anti-piratage). `VideoStorageGateway::getDownloadState()` (Cloudflare Stream "Enable MP4 downloads"), `POST /api/videos/{video}/download` réservé aux vidéos achetées et prêtes. Mobile : `DownloadManager`, stockage sandboxé (`path_provider`, jamais dans la galerie média), `DownloadButton` sur la fiche vidéo, lecture locale sans réseau si le fichier est déjà téléchargé.
- **Canal support viewer** : généralise le système créateur ↔ modération existant plutôt que d'en construire un second — `messages.creator_id` renommé `user_id`, routes `/api/creator/messages` → `/api/messages` (partagées), action Filament "Messagerie" sur `/moderation/users` ouverte à tous les rôles sauf modérateur. Web : nouvelle page `/support` ; mobile : bouton "Contacter le support" sur "Mon compte" en plus de l'espace créateur.

**Back-office modération enrichi** (2026-09-05/06/11) : logo StreamMali dans la barre du haut (clair/sombre) ; export Excel des transactions et des comptes (via le système d'export natif Filament, export forcé en synchrone faute de worker de queue en prod) ; onglets rapides sur `/moderation/videos` (Toutes/En attente/Signalées/Validées/Refusées, "Toutes" par défaut — une vidéo signalée était déjà publique) et filtres par colonne complétés sur `/moderation/videos` (fichier vidéo, en vedette, prix, date de soumission) et sur `/moderation/ledger-entries` (vidéo, montants, ID transaction, date, ID) ; colonnes par défaut réduites sur `/moderation/videos` pour éviter le scroll horizontal (le reste togglable via le menu Filament). `provider_transaction_id` (renommé de "Référence PayDunya" à "ID transaction") est maintenant renseigné par le webhook Orange Money (`txnid`), plus utile pour la réconciliation.

**Mosaïque de jaquettes dans le hero** (web + mobile, 2026-09-06) : remplace l'aplat de couleur unie (palette empruntée à PayDunya, lisait "app fintech" plutôt que "cinéma") par un fond composé des jaquettes en vedette (repli sur le catalogue général, puis sur l'aplat si aucune jaquette). Fortement flouté après retour utilisateur — le contenu de test actuel (rediffusions TikTok) n'était pas présentable en clair ; s'estompera de lui-même avec de vraies jaquettes.

**Coordonnées de contact** (2026-09-08) : colonne "Contact" au footer web et bloc équivalent sur "Mon compte" mobile (`contact@streammali.ml`, `+223 78 98 11 88`). En profite pour purger les dernières mentions de `support@streammali.com` (jamais possédé) et de PayDunya comme sous-traitant de paiement dans les pages CGU/politique de confidentialité, restées de l'époque où PayDunya était le gateway actif.

Bugs de prod corrigés en cours de route : logo de modération manquant (contenu mixte `http://`, `asset()` figeait le schéma avant que `trustProxies` détecte le HTTPS via le proxy Render — remplacé par `config('app.url')`) ; 500 sur `/moderation` (colonne `notifications.data` en `text` plutôt que `json`, Postgres rejette l'opérateur JSON que SQLite tolérait par typage dynamique) ; texte illisible sur les notifications non lues en mode sombre. Suite : 167/167 backend, `tsc`/`lint`/`flutter analyze`/`test` propres.

**Vignettes noires + captures Play Store** (2026-09-12) : en préparant les captures d'écran de la fiche Play Store, `CloudflareStreamGateway` s'est révélé prendre la vignette par défaut de Cloudflare Stream (image à t=0) — un cadre noir pour toute vidéo avec un fade-in, touchant la moitié du catalogue de test en prod (JPEG noir identique, 1651 octets, sur 4 des 8 vidéos). Corrigé pour demander `?time=1s` explicitement (les nouvelles vidéos se corrigent seules ; les 4 déjà en base ont été patchées manuellement, `?time=20s` pour celle avec un carton d'intro plus long). Le flou volontaire du hero (web + mobile, voir plus haut) n'avait plus de raison d'être une fois ce bug corrigé — retiré, les jaquettes s'affichent nettes comme prévu à l'origine. 4 captures générées sur téléphone physique (accueil, catalogue, fiche vidéo, page créateur), voir `infra/play-store-assets/screenshots/` et `infra/PLAY_STORE_LISTING.md` — sur le contenu de test actuel (NASA/espace), à refaire une fois du vrai contenu malien publié.

**Upload vidéo : passage à TUS résumable, plus de plafond à 200 Mo** (2026-09-15) : l'upload direct Cloudflare Stream utilisait le flux "Basic" (une seule requête `multipart/form-data` avec le fichier entier), plafonné par Cloudflare à 200 Mo — trop petit pour la plupart des films, découvert en conditions réelles (échec avec un 413 à la première tentative d'upload d'un vrai film). Un commentaire plus ancien du code supposait que l'upload résumable (TUS) était impossible sans exposer le token API secret au navigateur/à l'app ; c'était faux — seule la création de l'URL d'upload côté serveur a besoin du token, l'URL à usage unique renvoyée gère l'auth toute seule pour les requêtes de chunks envoyées directement par le client. Remplacé sur les trois couches (API : `CloudflareStreamGateway` crée l'upload via `POST /stream?direct_user=true`, en-têtes `Tus-Resumable`/`Upload-Length`/`Upload-Metadata` plutôt que `POST /stream/direct_upload` ; web et mobile : upload par chunks de 50 Mo via `PATCH`, avec une requête `HEAD` pour récupérer l'offset réel avant de rejouer un chunk en échec plutôt que de supposer qu'aucun octet n'est arrivé côté serveur). Plus de plafond de taille, et upload plus robuste sur 3G/4G (retry par chunk au lieu de tout le fichier).

**Retour au sandbox Orange Money pour retester** (2026-09-15) : nouveau compte/app sandbox créé sur le Developer Center pour retester le flux de bout en bout. Piège rencontré au passage : `client_id`/`client_secret` copiés depuis deux entrées différentes (ou secret copié avec une erreur) → 401 `invalid_client` d'Orange à l'échange OAuth, masqué côté client par le 502 générique — vérifiable uniquement dans les logs Render. Voir la mémoire de session `project_orange_money_sandbox_gotchas` (point 9) pour le détail.

**Solde par créateur, transactions côté créateur, nettoyage des tirets cadratins** (2026-09-15) :
- Dashboard modérateur : nouveau stat "Solde du compte principal" (ventes encaissées moins retraits déjà payés — distinct de "Commission plateforme" qui ne fait qu'augmenter) et colonne "Solde" par créateur sur `/moderation/users`.
- Créateur : nouvel historique des transactions (`GET /api/creator/transactions`, une ligne par vente) sur `/creer/solde` (web) et `CreatorBalanceScreen` (mobile) — distinct de l'historique des demandes de retrait déjà existant.
- Tous les tirets cadratins (« — ») dans le texte visible par les utilisateurs (web + mobile — titres de page, contenu CGU/politique de confidentialité, textes d'onboarding, libellés de bouton) remplacés par une ponctuation plus neutre (virgule, deux-points, tiret simple pour les titres de page) — laissés tels quels dans les commentaires de code, non visibles.

**Prochaine étape — passage en production :**
Le flux Orange Money est désormais vérifié de bout en bout en sandbox (redirection, OTP, webhook, payload réel connu et géré correctement — voir plus haut). Ce qui reste avant la vraie mise en production : obtenir l'approbation du produit **de production** Orange Money (le sandbox "WebPay Dev" ne couvre que les tests, c'est une démarche/validation marchand séparée, comme le KYC PayDunya), puis repasser `ORANGE_MONEY_COUNTRY` sur le vrai code pays et refaire un paiement réel avec de l'argent réel pour confirmer que rien ne change de comportement hors sandbox. PayDunya reste une option de repli si son KYC aboutit avant. L'IP autorisée sur le token Cloudflare Stream devra couvrir l'IP de sortie de Render, pas seulement celle utilisée pour la vérification manuelle. En parallèle, la feuille de route produit continue (reste des items P2 : suivre un créateur, tri/réponse sur les avis, squelette i18n bambara), et la publication Play Store avance (voir `infra/PLAY_STORE_LISTING.md`).
