...
- `apps/web` : scaffold Next.js initialisé (TypeScript, Tailwind, App Router). Catalogue fonctionnel en SSR (liste + fiche détail + filtres), consomme `apps/api`, vérifié de bout en bout avec l'API réellement lancée. Bouton d'achat désactivé (auth client/CORS pas encore configurés).
