...
- `apps/mobile` : scaffold Flutter initialisé (`com.streammali`). Catalogue fonctionnel (liste paginée + filtres + fiche détail), consomme `apps/api`. `flutter analyze`/`flutter test`/`flutter build web` propres. Auth/achat pas encore portés côté mobile (prévus avec le même token Bearer Sanctum que le web).
