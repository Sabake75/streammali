...
Titre d'onglet par page + navigation active (WCAG 2.4.2/2.4.8) : chaque page a maintenant son propre `<title>` ("X — StreamMali") au lieu du "StreamMali" statique du layout racine partout. Piège rencontré : `document.title = ...` dans un `useEffect` (Client Component) ne marche pas sur cette version de Next — le titre est géré déclarativement par un mécanisme de streaming metadata interne (`Next.MetadataOutlet`, voir `node_modules/next/dist/docs/01-app/01-getting-started/14-metadata-and-og-images.md`) qui écrase toute mutation impérative peu après le montage. Solution : `export const metadata` ne fonctionne que dans un Server Component, donc les pages qui utilisent des hooks (`useState`/`useEffect`/`useRouter`...) sont désormais un `page.tsx` Server Component fin (juste `export const metadata` + rendu du composant client) qui délègue tout le contenu interactif à un fichier `*PageClient.tsx` colocalisé — les 4 pages `/creer/*` qui n'avaient aucun hook à leur propre niveau ont simplement perdu leur `"use client"`. `src/components/NavLink.tsx` : variante de `Link` qui s'auto-détecte active via `usePathname()` (`aria-current="page"` + classe active) — utilisée pour "Espace créateur", "Favoris", "Mes achats", la cloche de notifications et le nom du header.
