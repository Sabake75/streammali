...
export function VideoCard({ video }: { video: VideoSummary }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-lg border border-neutral-200/80 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_20px_-12px_rgba(0,0,0,0.12)] transition duration-200 hover:-translate-y-1 hover:border-orange-300 hover:shadow-[0_1px_2px_rgba(0,0,0,0.04),0_16px_32px_-12px_rgba(234,88,12,0.25)] active:scale-[0.98] active:border-orange-300 dark:border-neutral-800 dark:bg-neutral-950 dark:hover:border-orange-800"
    >
      <div
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <span className={`w-fit rounded-full px-2 py-0.5 text-xs font-semibold tracking-wide ${style.badge}`}>
          {video.category.label}
        </span>
        <h3 className="mt-1 line-clamp-2 text-sm font-semibold text-neutral-900 group-hover:text-orange-600 dark:text-neutral-50 dark:group-hover:text-orange-400">
          {video.title}
        </h3>
