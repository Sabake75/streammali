...
- Intégration paiement Mobile Money via Orange Money (Web Payment, gateway actif — voir `CLAUDE.md` à la racine du dépôt et `app/Domain/Payment/README.md` pour l'historique de bascule avec PayDunya, gardé en repli).
...
API fonctionnellement complète (voir `CLAUDE.md` à la racine pour l'état détaillé et l'historique) — les paragraphes ci-dessous décrivent ce qui est réellement implémenté, pas une intention. PostgreSQL en prod (`DB_CONNECTION=pgsql`), SQLite pour la suite de tests locale.

Les dossiers `Domain/{Creator,Viewer,Moderation,Payment,Video,Review}` ont chacun un README décrivant leur rôle et leur sous-structure (`Models/`, `Actions/`, `Data/`).
...
Table `payments` + modèle `App\Domain\Payment\Models\Payment`, intégration Orange Money (`App\Domain\Payment\Gateways\OrangeMoneyGateway`, gateway actif) — voir `app/Domain/Payment/README.md` pour le détail (dont les pièges d'intégration trouvés en réel). Config dans `.env`/`config/services.php` (`ORANGE_MONEY_*`). Webhook : `/api/webhooks/orange-money`.
...
Traductions françaises des messages de validation par défaut (`lang/fr/validation.php`) : repéré en production sur `/inscription-createur` — `APP_LOCALE=fr` et `APP_FALLBACK_LOCALE=fr` (voir `render.yaml`) sans fichier `lang/fr/` correspondant renvoient la clé de traduction brute ("validation.uploaded") au lieu d'un message, faute de fallback anglais possible. Deux bugs distincts trouvés en testant un vrai upload de pièce d'identité en conditions réelles (fichier > 2MB, taille normale pour une photo de téléphone) : `upload_max_filesize`/`post_max_size` par défaut de PHP (2M/8M) rejetaient le fichier avant même que la règle `max:10240` (10MB) de `RegisterCreatorController` ne s'applique — relevés à 12M/13M dans `Dockerfile`/`Dockerfile.prod` (`conf.d/uploads.ini`) ; et `display_errors=STDOUT` (défaut de l'image de base) faisait fuiter un warning PHP brut dans la réponse HTTP au-delà même de cette nouvelle limite, avec un statut 200 au lieu de 413 — corrigé (`display_errors=Off`, `log_errors=On` vers stderr) dans `Dockerfile.prod`, plus un handler dédié dans `bootstrap/app.php` pour traduire le message de `PostTooLargeException` (codé en dur en anglais dans Laravel, pas une clé `lang/`).

Trois autres bugs trouvés en reproduisant un vrai échec d'upload de pièce d'identité en production (Render) :
- `LOG_CHANNEL=stack` (défaut de `render.yaml`) revient au canal `single`, qui écrit dans `storage/logs/laravel.log` — un fichier **dans le conteneur éphémère**, invisible dans l'onglet Logs de Render (qui ne capture que stdout/stderr). Toutes les erreurs applicatives (pas seulement celle-ci) étaient donc silencieuses en production. Corrigé : `LOG_CHANNEL=stderr`.
- `league/flysystem-aws-s3-v3` n'était pas dans `composer.json` — package requis par Laravel pour le driver `s3` (pas installé par défaut), donc `FILESYSTEM_DISK=s3` n'a jamais pu fonctionner, avec ou sans les bonnes clés R2 (`Class "League\Flysystem\AwsS3V3\PortableVisibilityConverter" not found`). Ajouté (`composer require league/flysystem-aws-s3-v3`).
- Plus grave : `'throw' => false` (valeur par défaut de Laravel) sur les disques `local`/`s3` (`config/filesystems.php`) fait qu'un échec d'écriture renvoie silencieusement `false` au lieu de lever une exception — ni `RegisterCreator` ni `UpgradeToCreator` ne vérifient ce retour. Reproduit avec de fausses clés R2 : réponse **200/201** (compte transformé en créateur) avec `identity_document_path` enregistré comme `"0"` (cast string de `false`) — aucun fichier réellement stocké, aucune erreur visible. Corrigé : `'throw' => true` sur `local`/`s3`, plus un handler dans `bootstrap/app.php` pour `League\Flysystem\UnableToWriteFile` (message français, 502 — même traitement que les autres pannes de service externe).
